				...
				if (socketTest($results[$x]->server_name, $results[$x]->server_update_port, 10)) {
					...
					if ($results[$x]->server_update_method == 'ssh') {
						...
						if (!$ssh_key) {
							...
							exec(findProgram('ssh') . " -T -i " . escapeshellarg($temp_ssh_key) . " -o 'StrictHostKeyChecking no' -p " . escapeshellarg($results[$x]->server_update_port) . ' -l ' . escapeshellarg($ssh_user) . ' ' . escapeshellarg($results[$x]->server_name) . " 'ls " . escapeshellarg("/usr/local/$fm_name/{$_SESSION['module']}/client.php") . "'", $post_result, $retval);
							if ($retval) {
